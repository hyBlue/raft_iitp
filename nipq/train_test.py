...
def train_aux_target_avgbit(train_loader, model, model_ema, model_t, criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss_bit = bit_loss(model, epoch, bit_scale_a,
...
def train_aux(train_loader, model, model_ema, model_t, criterion, optimizer, epoch, ema_rate=0.9999, bit_scale_a=0, bit_scale_w=0):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss_class = loss_class + bit_scale_a * loss_bit_a / \
            2 ** 23 + bit_scale_w * loss_bit_w / 2 ** 23
...
def get_optimizer(weight, bnbias, train_weight):
    optimizer = optim.SGD([
        ...
        {'params': weight, 'weight_decay': args.decay if train_weight else 0,
            'lr': args.lr if train_weight else 0},
