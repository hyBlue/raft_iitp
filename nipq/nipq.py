            ...
            with torch.no_grad():
                ...
                if hist.sum() != 0:
                    noise_t = torch.multinomial(
                        hist, data.numel(), True) + torch.rand_like(data.reshape(-1))
...
class Q_Conv2d(nn.Conv2d):
    def __init__(self, *args, act_func=None, mask=None, **kargs):
        ...
        if mask:
            self.mask = nn.Parameter(torch.ones_like(
                self.weight.data).cuda().flatten(), requires_grad=False)
        else:
            self.mask = None
